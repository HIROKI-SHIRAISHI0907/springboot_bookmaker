...
public class ForceAdminAPIService {
	 ...
	 */
	@Transactional
    public ForceAdminResponse upsert(String country, String league, String team) {
		ForceAdminResponse res = new ForceAdminResponse();
		int result1 = repoSeason.updateDelFlgOne(country, league);
        if (result1 == 0) {
        ...
        int result2 = repoMaster.updateDelFlgOne(country, league, team);
        if (result2 == 0) {
        	res.setResponseCode("9");
    	    res.setMessage("更新エラー");
    	    return res;
