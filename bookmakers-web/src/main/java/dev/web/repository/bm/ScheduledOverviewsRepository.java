...
public class ScheduledOverviewsRepository {
	...
	public ScheduledSurfaceSnapshotDTO findLatestSnapshot(
		...
		String sql = """
				...
				monthly AS (
				  SELECT
				    -- 年月は int として使うが、文字列格納でも動くように ::text 経由
				    NULLIF(BTRIM(game_year::text),  '')::int  AS game_year,
				    NULLIF(BTRIM(game_month::text), '')::int  AS game_month,

				    CASE WHEN :role = 'home'
				      THEN COALESCE(NULLIF(BTRIM(home_sum_score::text),   '')::int, 0)
				      ELSE COALESCE(NULLIF(BTRIM(away_sum_score::text),   '')::int, 0)
				    END AS goals_for_m,

				    CASE WHEN :role = 'home'
				      THEN COALESCE(NULLIF(BTRIM(home_clean_sheet::text), '')::int, 0)
				      ELSE COALESCE(NULLIF(BTRIM(away_clean_sheet::text), '')::int, 0)
				    END AS clean_sheets_m,

				    CASE WHEN :role = 'home'
				      THEN COALESCE(NULLIF(BTRIM(home_1st_half_score::text), '')::int, 0)
				      ELSE COALESCE(NULLIF(BTRIM(away_1st_half_score::text), '')::int, 0)
				    END AS first_half_m,

				    CASE WHEN :role = 'home'
				      THEN COALESCE(NULLIF(BTRIM(home_2nd_half_score::text), '')::int, 0)
				      ELSE COALESCE(NULLIF(BTRIM(away_2nd_half_score::text), '')::int, 0)
				    END AS second_half_m,

				    CASE WHEN :role = 'home'
				      THEN COALESCE(NULLIF(BTRIM(home_first_goal_count::text), '' )::int, 0)
				      ELSE COALESCE(NULLIF(BTRIM(away_first_goal_count::text), '' )::int, 0)
				    END AS first_goal_m,

				    CASE WHEN :role = 'home'
				      THEN COALESCE(NULLIF(BTRIM(home_win_behind_count::text), '' )::int, 0)
				      ELSE COALESCE(NULLIF(BTRIM(away_win_behind_count::text), '' )::int, 0)
				    END AS win_behind_m,

				    CASE WHEN :role = 'home'
				      THEN COALESCE(NULLIF(BTRIM(home_lose_behind_count::text), '' )::int, 0)
				      ELSE COALESCE(NULLIF(BTRIM(away_lose_behind_count::text), '' )::int, 0)
				...
				agg AS (
				  SELECT
				    -- games / win / draw / lose / winning_points も text で来る可能性があるので文字列→int 変換
				    SUM(COALESCE(NULLIF(BTRIM(games::text),  '')::int, 0))::int            AS games,
				    SUM(COALESCE(NULLIF(BTRIM(win::text),    '')::int, 0))::int            AS win,
				    SUM(COALESCE(NULLIF(BTRIM(draw::text),   '')::int, 0))::int            AS draw,
				    SUM(COALESCE(NULLIF(BTRIM(lose::text),   '')::int, 0))::int            AS lose,
				    SUM(COALESCE(NULLIF(BTRIM(winning_points::text), '')::int, 0))::int   AS winning_points,
				    ...
				    SUM(
				      CASE WHEN :role = 'home'
				            THEN COALESCE(NULLIF(BTRIM(home_win_count::text), '' )::int, 0)
				            ELSE COALESCE(NULLIF(BTRIM(away_win_count::text), '' )::int, 0)
				      END
				    )::int AS win_count_role,

				    SUM(
				      CASE WHEN :role = 'home'
				            THEN COALESCE(NULLIF(BTRIM(home_lose_count::text), '' )::int, 0)
				            ELSE COALESCE(NULLIF(BTRIM(away_lose_count::text), '' )::int, 0)
				      END
				    )::int AS lose_count_role,

				    -- 役割に依存しない
				    SUM(
				      COALESCE(NULLIF(BTRIM(fail_to_score_game_count::text), '' )::int, 0)
				    )::int AS fail_to_score_game_count
				  FROM base b
				  JOIN monthly m
				    ON NULLIF(BTRIM(b.game_year::text),  '')::int = m.game_year
				   AND NULLIF(BTRIM(b.game_month::text), '')::int = m.game_month
				),
				...
				latest AS (
				  ...
				  FROM base
				  ORDER BY
				    NULLIF(BTRIM(game_year::text),  '')::int  DESC,
				    NULLIF(BTRIM(game_month::text), '')::int  DESC
				  LIMIT 1
				)
				...
				SELECT
				  ...
				  latest.last_week_game_win_disp, latest.lose_streak_disp, latest.promote_disp, latest.first_win_disp,
				  latest.descend_disp, latest.home_adversity_disp, latest.away_adversity_disp
