		...
		try (BufferedReader br = new BufferedReader(
			...
			while ((text = br.readLine()) != null) {
				...
				if (row > 0) {
					...
					mappingDto.setHomeClearCount(parts[60]);
					mappingDto.setAwayClearCount(parts[61]);
					mappingDto.setHomeDuelCount(parts[62]);
					mappingDto.setAwayDuelCount(parts[63]);
					mappingDto.setHomeInterceptCount(parts[64]);
					mappingDto.setAwayInterceptCount(parts[65]);
					mappingDto.setRecordTime(parts[66]);
					mappingDto.setWeather(parts[67]);
					mappingDto.setTemperature(parts[68]);
					...
					mappingDto.setJudge(parts[93]);
					mappingDto.setHomeTeamStyle(parts[94]);
					mappingDto.setAwayTeamStyle(parts[95]);
					mappingDto.setProbablity(parts[96]);
					mappingDto.setPredictionScoreTime(parts[97]);
