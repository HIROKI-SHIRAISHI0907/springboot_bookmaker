...
public class ReadTeam {
	...
	public ReadFileOutputDTO getFileBody(String fileFullPath) {
		...
		List<CountryLeagueMasterEntity> entiryList = new ArrayList<CountryLeagueMasterEntity>();
		try (InputStream is = new FileInputStream(new File(fileFullPath));
			...
			if (wb.getNumberOfSheets() == 0) {
				...
				readFileOutputDTO.setCountryLeagueMasterList(entiryList);
			...
			for (int r = firstRow + 1; r <= lastRow; r++) { // 1行目をヘッダーとしてスキップ
				...
				String team = cellString(row.getCell(2), formatter);
				String path = cellString(row.getCell(3), formatter);
