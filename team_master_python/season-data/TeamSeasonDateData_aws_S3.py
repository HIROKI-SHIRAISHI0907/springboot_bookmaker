# -*- coding: utf-8 -*-
"""
Flashscore から season_data.csv を更新して S3 に保存（ECS Fargate / Lambda 兼用）

要件反映：
- 出力: s3://aws-s3-season-csv/season_data.csv
- 入力JSON: s3://aws-s3-season-csv/json/b001_country_league.json
"""
...
# S3（固定）
S3_BUCKET = os.getenv("S3_BUCKET", "aws-s3-season-csv")
SEASON_S3_KEY = "season_data.csv"
...
async def click_show_more_fixtures_until_end(page, button_text="もっと試合を表示する", max_clicks=120, wait_timeout=15000) -> int:
    clicks = 0
    while clicks < max_clicks:
...
async def main():
    ...
    # 1) 既存CSVをS3から復元（あれば）
    s3_download_if_exists(SEASON_S3_KEY, SEASON_CSV)
    ...
    async with async_playwright() as p:
        ...
        s3_upload_file(SEASON_CSV, SEASON_S3_KEY)
